...
def downsample(info: WAVInfo, factor: int, verbose=False) -> WAVInfo:
    ...
    if verbose:
        logger.info(f"Downsampling by factor of {factor}...")
...
def downmixToMono(info: WAVInfo, verbose=False) -> WAVInfo:
    if info.mono:
        return info
    if verbose:
        logger.info("Downmixing stereo to mono...")
    ...
    if len(data) % 2 != 0:
        if verbose:
            logger.info("Warning: Data length is not even, removing the last sample...")
    ...
    if verbose:
        logger.info("Downmixing complete.")
        # printInfo(info)
    return info
...
def lowpassFilter(info: WAVInfo, cutoff: float, verbose=False) -> WAVInfo:
    if verbose:
        logger.info(f"Applying lowpass filter with cutoff frequency of {cutoff} Hz...")
    ...
    if verbose:
        logger.info("Lowpass filter complete.")
        # printInfo(info)
    return info
