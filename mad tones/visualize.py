...
def visualizeSong(
    ...
    plt.grid(True)
    if save:
        plt.savefig(f"{id}_waveform.png")
    plt.show()


def visualizeStrongestFrequencies(times, freqs, id, save=False):
    ...
    plt.grid(True)
    if save:
        plt.savefig(f"{id}_strongest.png")
    plt.show()


def visualizeSpectograph(
    freq, times, Zxx, data, overlap, windowSize, sampleFreq, id, save=False
):
    ...
    if save:
        plt.savefig(f"{id}_spectrogram.png")
    return freq, times, Zxx


def visualizeSpectographFromInfo(info: WAVInfo, id, factor=1, freqRes=10.7, save=False):
